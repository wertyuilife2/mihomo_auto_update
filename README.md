...
## TODO
1. 开启start_offline.sh后，./kill.sh似乎杀不掉mihomo进程，进程名字匹配有问题？

2. 在h200和跳板机上试了一下start_offline.sh，似乎不work，h200的google.com dns解析失败，跳板机变得非常慢然后卡死了。感觉还是不太敢在这些机器上实验了。不知道是不是触发了什么循环代理，还是需要进一步在完全没有网的机器上测试：代理后新建ssh，然后进行一下下载之类的任务，看看本机的clash客户端的日志会不会有问题。以及进行新的ssh，并执行kill.sh之类的，以及断开老的ssh之类的。总体上感觉还是dns设置的问题。

3. 似乎在TUN模式下直接kill -9 mihomo进程会导致一些问题，比如变成僵尸进程之类的，可能需要ip link delete Meta 2>/dev/null。在kill之后删除虚拟网卡。
