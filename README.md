...
1. 首先，在`config.conf`的`SUB_URL`项填写你的订阅地址。`config.conf` 会根据自身位置自动确定 `REPO_ROOT`，各脚本优先用它拼接仓库内路径。
...
1. 配置`config_offline.yaml`中的`mylocalproxy`的`port`，与你配置的局域网代理服务端口一致。
...
- 设置 `config_offline.yaml` 中 `mylocalproxy` 的 `server` 和 `port` 分别为代理服务的地址和端口
- 使用 `./start_offline.sh`
